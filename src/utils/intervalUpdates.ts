 ...
 * @param event
 */
export function updateSkyXDayData(event: ethereum.Event): SkyXDayData {
  let skyx = Factory.load(FACTORY_ADDRESS) as Factory
  let timestamp = event.block.timestamp.toI32()
  let dayID = timestamp / 86400 // rounded
  let dayStartTimestamp = dayID * 86400
  let skyxDayData = SkyXDayData.load(dayID.toString())
  if (skyxDayData === null) {
    skyxDayData = new SkyXDayData(dayID.toString())
    skyxDayData.date = dayStartTimestamp
    skyxDayData.volumeETH = ZERO_BD
    skyxDayData.volumeUSD = ZERO_BD
    skyxDayData.volumeUSDUntracked = ZERO_BD
    skyxDayData.feesUSD = ZERO_BD
  }
  skyxDayData.tvlUSD = skyx.totalValueLockedUSD
  skyxDayData.txCount = skyx.txCount
  skyxDayData.save()
  return skyxDayData as SkyXDayData
}
